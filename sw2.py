...
length_scale = 3.086e25 # mega parsec
INTEGRATOR = 'vode'
INTEGRATOR_PARAMS = {
    'atol': 1e-100, 
    'rtol': 1e-15,
...
def solve(angle_to_horizontal, comoving_lens=1e25, plot=True):
    ...
    initial_r = 0
    initial_phi = angle_to_horizontal
    initial_t = 0.
    ...
    initial_rdot = comoving_lens
    initial_phidot = 0
    initial_R = initial_a*initial_r
    initial_tdot = -np.sqrt(initial_a**2*initial_rdot**2 + initial_R**2*initial_phidot**2)
    ...
    while solver_frw.successful():
        ...
        distance_from_lens = np.sqrt((solver_frw.y[1]*np.sin(solver_frw.y[4]))**2 + (solver_frw.y[1]*np.cos(solver_frw.y[4])-comoving_lens)**2)
        # distance_from_lens = np.sqrt((solver_frw.y[1]-comoving_lens)**2+solver_frw.y[1]*solver_frw.y[4])
        if distance_from_lens <= r_h:
            last = solver_frw.y
            break
    ...
    # convert to frw coordinates in the centre of the hole
    print("conversion...")
    print(last)
    new_last = last.copy()
    new_x = last[1]*np.cos(last[4]) - comoving_lens
    new_y = last[1]*np.sin(last[4])
    new_last[1] = np.sqrt(new_x**2 + new_y**2)
    new_last[4] = np.arccos(new_x / new_last[1])
    prev_phidot = last[2]/new_last[1]*np.sin(last[4] - new_last[4])
    print(prev_phidot)
    # new_last[2] = np.sqrt(last[2]**2 - new_last[1]**2*prev_phidot**2)
    new_last[2] = last[2]*np.cos(last[4] - new_last[4])
    last = new_last
    print(last)
    ...
    tdot_out = -np.sqrt(last[0]**2*last[2]**2 + last[0]**2*last[1]**2*prev_phidot)
    initial_t = 0
    initial_r = r_out
    initial_phi = last[4]
    ...
    # convert back to FRW coordinates at observer
    new_initial_frw2 = initial_frw2.copy()
    new_x = initial_frw2[1] * np.cos(initial_frw2[4]) + comoving_lens
    new_y = initial_frw2[1] * np.sin(initial_frw2[4])
    new_initial_frw2[1] = np.sqrt(new_x**2 + new_y**2)
    new_initial_frw2[4] = np.arcsin(new_y / new_initial_frw2[1])
    initial_frw2 = new_initial_frw2
    ...
    if plot:
        ...
        axes.set_ylim([-lim, lim])
    return a[-1]*r[-1]
    # return (r[-1] + comoving_lens)*a[-1] - 196.7608578559894
...
def main():
    ...
    ds = []
    for theta in thetas:
        result = solve(theta, plot=True, comoving_lens=comoving_lens)
        ds.append(result)
    ds = np.array(ds)
