'''
origin at lens
fixed start_theta, fixed mass, varying z_lens, varying lambda
'''
...
def main():
    ...
    step_size = 1e-07
    first = True
    for z_lens, start_th in tqdm(list(zip(z_lens_all, start_thetas))):
        rs = []
        thetas = []
        dl = []
        dls = []
        ...
        filename = 'data/diff_lambdas_redshifts.csv'
        if first:
