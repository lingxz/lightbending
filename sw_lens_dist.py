...
def main():
    ...
    z_lens_all = np.linspace(0.05, 0.2, 400)
    for om in tqdm(om_lambdas): 
        ...
        df = pd.DataFrame({'lens_z': z_lens_all, 'percentage_err': percentage_errors})
        df.to_csv('data/lens_z_omlambda_{0:.5f}.csv'.format(om), index=False)
    
    # print(percentage_errors)
    print("Time taken: {}".format(time.time() - start))
    # plt.plot(z_lens_all, percentage_errors, 'r+')
